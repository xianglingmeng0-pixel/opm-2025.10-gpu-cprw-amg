...
CASE=${CASE:-/workspace/cases/spe10/SPE10-MOD02-01.DATA}
...
./bin/flow "$CASE" \
  ...
  | tee "$LOGDIR/log_spe10_gpuilu0_tol005_max200_1mpi12t_full.log"
...
./bin/flow "$CASE" \
  ...
  | tee "$LOGDIR/log_spe10_gpudilu_tol005_max200_1mpi12t_full.log"
...
./bin/flow "$CASE" \
  ...
  | tee "$LOGDIR/log_spe10_gpu_cpr_1mpi12t.log"
...
./bin/flow "$CASE" \
  ...
  | tee "$LOGDIR/log_spe10_gpu_cprw_1mpi12t.log"
...
mpirun --allow-run-as-root \
  ...
  | tee "$LOGDIR/log_spe10_cpu_cpr_12mpi1t.log"
...
mpirun --allow-run-as-root \
  ...
  | tee "$LOGDIR/log_spe10_cpu_cprw_12mpi1t.log"
